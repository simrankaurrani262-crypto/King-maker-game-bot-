"""
Map System Handler - Full map rendering and tile interaction.
Fixed version with proper imports at top.
"""
...
async def show_tile_detail(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, x: int, y: int):
    ...
    army_total = 0
    if occupant.army:
        army_total = (getattr(occupant.army, 'infantry', 0) +
                      getattr(occupant.army, 'archers', 0) +
                      getattr(occupant.army, 'cavalry', 0))

    text = f"""👑 **{occupant.name}** {occupant.flag}
━━━━━━━━━━━━━━
🏆 Level: {occupant.level}
⚔️ Army: ~{army_total} (estimated)
🛡 Defense: {get_defense_rating_label(defense_power)}
🟢 Status: {'Online' if getattr(occupant, 'is_online', False) else 'Offline'}
